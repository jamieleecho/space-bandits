...
def create_tile_and_tile_map_files(
    tiles,
    tile_file_path,
    tile_image,
    image_path,
    tile_map_file_path=None,
    cmp=False,
    image_size=None,
):
    """
    ...
    rgb8_to_palette = (
        coco.RGB8_TO_COCO_CMP_COLORS if cmp else coco.RGB8_TO_COCO_RGB_COLORS
    )
    ...
    tile_to_index = {tile: ii for ii, tile in enumerate(tile_list)}
...
parser = argparse.ArgumentParser(
    description="Create tile and tile map files from an image"
)
parser.add_argument(
    "image_path", type=str, help="Path to image used to create tile and tile map"
)
parser.add_argument("tile_file_path", type=str, help="Path to output tile file")
parser.add_argument(
    "--threshold", type=float, default=0.05, help="Threshold to use for comparing tiles"
)
parser.add_argument(
    "--num-colors", type=int, default=16, help="Maximum number of colors to use"
)
parser.add_argument(
    "--dither-reduce-mode",
    type=int,
    default=0,
    help="Dither mode to use when reducing colors",
)
parser.add_argument(
    "--dither-remap-mode",
    type=int,
    default=1,
    help="Dither mode to use when remapping colors",
)
parser.add_argument(
    "--cmp", default=False, action="store_true", help="Optimize for CMP palette"
)
...
with Image(filename=args.image_path) as img:
    ...
    wand_util.remap_colors(
        img, coco.create_color_map_image(cmp=args.cmp), dither=args.dither_remap_mode
    )
    ...
    with new_image.convert("GIF") as new_converted_image:
        ...
        create_tile_and_tile_map_files(
            tiles,
            args.tile_file_path,
            new_converted_image,
            image_path,
