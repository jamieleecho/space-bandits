...
- (void)loadImagesForTransitionInfo:(NSMutableArray <DSTransitionSceneInfo *>*)info {
    ...
    for(NSString *path in paths) {
        ...
        NSAssert([levelFilenameRegex numberOfMatchesInString:path.lastPathComponent options:0 range:NSMakeRange(0, path.lastPathComponent.length)] == 1, ([NSString stringWithFormat:@"Unexpected image filename %@", path.lastPathComponent]));
        ...
        NSAssert([levelToPath objectForKey:[NSNumber numberWithInt:level]] == nil, ([NSString stringWithFormat:@"Multiple images files found for level %d", level]));
    ...
    NSAssert(levels.count >= 1, @"No image files found.");
    NSAssert([levels[0] isEqualToNumber:@0], ([NSString stringWithFormat:@"First image starts at level %@ not level 0", levels[0]]));
    NSAssert([levels[levels.count - 1] isEqualToNumber:[NSNumber numberWithUnsignedLong:levels.count - 1]], ([NSString stringWithFormat:@"Last image should be for level %lu not level %@", levels.count, levels[levels.count - 1]]));
    NSAssert(levels.count == info.count, ([NSString stringWithFormat:@"Found %lu image files, but expecting %lu level images", levels.count, info.count]));
